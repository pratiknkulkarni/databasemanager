...
| `--force` | bool | `false` | Skip the interactive confirmation. Without it, the command prints exactly what will be deleted (database, user, app, environment) to **stderr** and requires `y`/`yes` on stdin. Anything else aborts safely. |
...
| `DB_PASSWORD` | `4f2b…` (32 hex) | The app's password |
...
| Gap | Detail |
|---|---|
| **Credential rotation** | No `rotate` command. Rotating an app's password today means manual `ALTER USER`/`ALTER ROLE` + editing the Infisical secret. A `rotate <app>` that regenerates the password, applies it, and updates `DB_PASSWORD` atomically would complete the lifecycle story. |
| **Idempotent / resumable provisioning** | `database already exists` is fatal and the secret sync uses batch **create** (no upsert). If provisioning succeeds but the sync fails, there is no `--adopt`/`--resync` to reconcile — you must delete and re-provision. |
